 ...
 */
class Version20180528122024 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql('DROP INDEX IDX_9474526C3C216F9D');
        $this->addSql('DROP INDEX IDX_9474526CA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__comment AS SELECT id, user_id, attraction_id, note, contenu FROM comment');
        $this->addSql('DROP TABLE comment');
        $this->addSql('CREATE TABLE comment (id INTEGER NOT NULL, user_id INTEGER DEFAULT NULL, attraction_id INTEGER DEFAULT NULL, note INTEGER NOT NULL, contenu CLOB NOT NULL COLLATE BINARY, PRIMARY KEY(id), CONSTRAINT FK_9474526CA76ED395 FOREIGN KEY (user_id) REFERENCES app_users (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_9474526C3C216F9D FOREIGN KEY (attraction_id) REFERENCES app_attraction (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO comment (id, user_id, attraction_id, note, contenu) SELECT id, user_id, attraction_id, note, contenu FROM __temp__comment');
        $this->addSql('DROP TABLE __temp__comment');
        $this->addSql('CREATE INDEX IDX_9474526C3C216F9D ON comment (attraction_id)');
        ...
        $this->addSql('INSERT INTO app_attraction (id, type_id, libelle, taillemini, age, description, image) SELECT id, type_id, libelle, taillemini, age, description, image FROM __temp__app_attraction');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__comment AS SELECT id, user_id, attraction_id, note, contenu FROM comment');
        $this->addSql('DROP TABLE comment');
        $this->addSql('CREATE TABLE comment (id INTEGER NOT NULL, user_id INTEGER DEFAULT NULL, attraction_id INTEGER DEFAULT NULL, note INTEGER NOT NULL, contenu CLOB NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO comment (id, user_id, attraction_id, note, contenu) SELECT id, user_id, attraction_id, note, contenu FROM __temp__comment');
        $this->addSql('DROP TABLE __temp__comment');
        $this->addSql('CREATE INDEX IDX_9474526CA76ED395 ON comment (user_id)');
        $this->addSql('CREATE INDEX IDX_9474526C3C216F9D ON comment (attraction_id)');
